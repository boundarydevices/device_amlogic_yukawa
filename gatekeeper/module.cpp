...
#define LOG_TAG "YukawaGatekeeper"
#include <log/log.h>
...
using yukawa::SoftGateKeeperDevice;

struct yukawa_gatekeeper_device {
    gatekeeper_device device;
    SoftGateKeeperDevice *s_gatekeeper;
};

static yukawa_gatekeeper_device s_device;
...
static int enroll(const struct gatekeeper_device *dev __unused, uint32_t uid,
    ...
    SoftGateKeeperDevice *s_gatekeeper = ((yukawa_gatekeeper_device*)(dev))->s_gatekeeper;
...
static int yukawa_gatekeeper_open(const hw_module_t *module, const char *name,
        hw_device_t **device) {
